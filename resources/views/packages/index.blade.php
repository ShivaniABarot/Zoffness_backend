    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="packagesTable" class="table table-striped table-hover datatable">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>Name</th>
                    ...
                    <tbody>
                        @forelse($packages as $package)
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-label-primary">{{ $package->number_of_sessions }}</span>
                                </td>
                                <td>{{ Str::limit($package->description, 100) }}</td>
                                <td class="text-center">
                                    <div class="d-inline-flex">
                                        <a href="{{ route('packages.show', $package->id) }}" class="btn btn-sm btn-icon btn-action-view" title="View">
                                            <i class="bx bx-show"></i>
                                        </a>
                                        <a href="{{ route('packages.edit', $package->id) }}" class="btn btn-sm btn-icon btn-action-edit" title="Edit">
    ...
    function deletePackage(packageId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '{{ route('packages.delete', '') }}/' + packageId,
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}',
                    },
...
<script>
    $(document).ready(function() {
        ...
        initDataTable('packagesTable', {
            // Any custom options specific to this table
            order: [[1, 'asc']], // Sort by name column
            columnDefs: [
                { width: "5%", targets: [0] },       // Make # column narrow
                { width: "25%", targets: [1] },      // Name column width
                { width: "10%", targets: [2] },      // Price column width
                { width: "10%", targets: [3] },      // Sessions column width
                { width: "35%", targets: [4] },      // Description column width
                { width: "15%", targets: [5] },      // Actions column width
                { className: 'fw-semibold', targets: [2] }, // Make price column bold
                { className: 'text-center', targets: [3] }, // Center the sessions badges
