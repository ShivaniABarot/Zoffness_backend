...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2><i class="bx bx-book-alt"></i> Executive Function Packages</h2>
    ...
    <div class="card p-3 shadow-sm">
        ...
        <table id="executiveFunctionTable" class="table table-striped table-bordered w-100">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Name</th>
...
<style>
    ...
    #executiveFunctionTable { border-radius: 10px; overflow: hidden; }
...
<script>
    $(document).ready(function () {
        if (!$.fn.DataTable.isDataTable('#executiveFunctionTable')) {
            $('#executiveFunctionTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route("executive_function_packages.data") }}',
                columns: [
    ...
    function deletePackage(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('executive_function_packages.delete', ':id') }}".replace(':id', id),
                    type: 'DELETE',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function (response) {
                        if (response.success) {
                            Swal.fire('Deleted!', response.message, 'success');
                            $('#executiveFunctionTable').DataTable().ajax.reload();
                        } else {
    ...
    function toggleStatus(id, currentStatus) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('executive_function_packages.toggleStatus', ':id') }}".replace(':id', id),
                    method: 'POST',
                    data: { _token: '{{ csrf_token() }}' },
                    success: function(response) {
                        if (response.success) {
