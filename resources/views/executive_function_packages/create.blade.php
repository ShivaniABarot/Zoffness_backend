            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-primary text-white text-center rounded-top">
                    <h4 class="mb-0 text-white">
                        <i class="bi bi-journal-plus me-2"></i>Create Executive Admissions Package
                    </h4>
                </div>

                <div class="card-body p-4">
                    <form id="createExecForm">
                        @csrf
...
<script>
    ...
    $(document).ready(function () {
        $('#createExecForm').on('submit', function (e) {
            e.preventDefault();

            $('#nameError, #priceError, #descriptionError').text('');

            $.ajax({
                url: '{{ route('executive_function_packages.store') }}',
                method: 'POST',
                data: $(this).serialize(),
                success: function (response) {
                    Swal.fire({
                        title: 'Success!',
                        text: 'Executive package created successfully.',
                        icon: 'success',
